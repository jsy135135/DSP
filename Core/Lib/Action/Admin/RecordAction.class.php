...
//            die();
            ...
            $D = M("data_dealed");
            $agent = M('recording_log', '','mysql://root:anlaigz@192.168.200.144:3306/vicidial');
...
//                $list[$i]["yuyin"] = "/file_path.php?date=$date&&tel=$phone";
                $yuyin = $agent->query("select location from recording_log where filename like '%".$phone."%'");
                $list[$i]["yuyin"] = $yuyin[0]['location'];
            }
//            var_dump($list);
